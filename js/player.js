 ...
 * @returns void
 */
function showEpisodesByControls(isShow) {
    if (isShow) {
        art.controls.update({
            name: 'select-episode',
            index: 1,
...
function renderEpisodes() {
    ...
    // 同时更新控制器中的集数按钮
    // 非全屏或非页面全屏时有问题，所以当前只在全屏或页面全屏时更新
...
// 渲染控制器中的集数按钮
function renderEpisodesByController() {
    return `<div class="relative group/xj">
                    <span class="cursor-pointer">选集</span>
                    <div
                        class="absolute w-[400px]  hidden group-hover/xj:block bottom-0 right-0 translate-x-1/2 pb-[50px] ">
                        <div class="w-full max-h-[400px] overflow-y-auto bg-[rgba(31,31,31,.9)] text-white px-4 py-2 bg-[#222] border border-[#333] rounded-lg transition-colors">
                            <div class="w-full grid grid-cols-6 gap-3">${currentEpisodes.map((episode, index) => {
        const isSelected = index === currentEpisodeIndex;
        return `<button onclick="playEpisode(${index})" class="p-2 aspect-square bg-[rgba(255,255,255,.05)] ${isSelected && "episode-active"} hover:bg-[rgba(255,255,255,.2)] border border-[#333] rounded-lg transition-colors">${index + 1}</button>`
    }).join('')
        }</div>
                        </div>
                    </div>
